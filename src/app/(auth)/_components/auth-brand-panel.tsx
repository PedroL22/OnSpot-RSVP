...
export const AuthBrandPanel = ({ description, title }: AuthBrandPanelProps) => {
  return (
    <div className='relative hidden overflow-hidden border-border border-r bg-card/70 lg:flex lg:w-[45%] xl:w-[42%]'>
      <div className='absolute inset-0 bg-[linear-gradient(var(--color-border)_1px,transparent_1px),linear-gradient(90deg,var(--color-border)_1px,transparent_1px)] opacity-35 [background-size:40px_40px]' />
      ...
      <div className='relative z-10 flex w-full flex-col justify-between p-12 xl:p-16'>
        ...
        <div className='space-y-8'>
          <div className='space-y-6'>
            <Eyebrow withDot>Authentication</Eyebrow>
            <DisplayTitle className='max-w-xs' size='xl'>
              {title}
            </DisplayTitle>
        ...
        <div className='flex items-center gap-3 font-mono text-[10px] text-muted-foreground uppercase tracking-[0.2em]'>
          <ShieldCheckered className='size-4 text-primary' />
          Secure auth
        </div>
      </div>
    </div>
